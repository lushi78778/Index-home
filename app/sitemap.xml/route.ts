...
export function GET() {
  ...
  const publishedCount = getAllPosts({ includeDraft: false }).filter(p => new Date(p.date).getTime() <= Date.now()).length
  ...
  const pageUrls = Array.from({ length: includePages }, (_, i) => i + 2)
    .map((n) => `<url><loc>${siteConfig.url}/blog/page/${n}</loc><lastmod>${now.toISOString()}</lastmod><changefreq>weekly</changefreq><priority>0.4</priority></url>`) 
    .join('\n')
